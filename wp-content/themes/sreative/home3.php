...
<?php
	...
	if ( function_exists( 'ot_get_option') ) {
		...
		/* Intro */
		$intro_h1 = ot_get_option( 'intro_h1' );
		/* MP4 */
        if ( function_exists( 'ot_get_option') ) {
				if(ot_get_option( 'mp4_h_bg' )!=""){
				   $mp4_h_bg = ot_get_option( 'mp4_h_bg' );
				   }else{
				   $mp4_h_bg = $templateurl.'/videos/video.mp4';
				  } 				
			}
		/* Services block */
...
<section class="home-intro">
  <video src="<?php echo esc_attr($mp4_h_bg) ?>" loop muted autoplay></video>
  <div class="table text-center">
    <div class="inner">
      <div class="slide-content">
        <?php echo esc_attr($intro_h1) ?>
	  </div>
      <!-- end slide-content --> 
    </div>
    <!-- end inner --> 
